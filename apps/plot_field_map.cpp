...
int main(int argc, char* argv[]) {
    ...
    auto* hBy_xz = new TH2F("hBy_xz", "B_{y} in xz-plane (y=0);z [m];x [m];B_{y} [T]",
                             nZ_xz, 0.0, 100.0, nX_xz, -3.5, 3.5);
    ...
    auto* hBy_yz = new TH2F("hBy_yz", "B_{y} in yz-plane (x=0);z [m];y [m];B_{y} [T]",
                             nZ_yz, 0.0, 100.0, nY_yz, -4.5, 4.5);
    ...
    auto* hBy_xy_ms =
        new TH2F("hBy_xy_ms", "B_{y} at MuonShield centre (z=16.76 m);x [m];y [m];B_{y} [T]",
                 nX_ms, -1.81, 1.81, nY_ms, -1.70, 1.70);
    for (int ix = 0; ix < nX_ms; ++ix) {
        ...
        for (int iy = 0; iy < nY_ms; ++iy) {
            ...
            hBy_xy_ms->SetBinContent(ix + 1, iy + 1,
                                     f[1].numerical_value_in(T));
        }
    }
    ...
    auto* hBy_xy_spec = new TH2F(
        "hBy_xy_spec", "B_{y} at Spectrometer centre (z=89.57 m);x [m];y [m];B_{y} [T]",
    ...
    for (int ix = 0; ix < nX_spec; ++ix) {
        ...
        for (int iy = 0; iy < nY_spec; ++iy) {
            ...
            hBy_xy_spec->SetBinContent(ix + 1, iy + 1,
                                       f[1].numerical_value_in(T));
        }
    }
